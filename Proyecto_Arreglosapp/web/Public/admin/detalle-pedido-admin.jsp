<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %> <%@ page import="dao.AdminDAO" %> <%@ page import="model.Usuario" %> <%@ page import="java.util.Map" %> <%@ page import="java.text.SimpleDateFormat" %> <% Usuario admin=(Usuario) session.getAttribute("usuario"); if (admin==null || admin.getRolId() !=1) { response.sendRedirect("/Proyecto_Arreglosapp/index.jsp"); return; } String pedidoIdStr=request.getParameter("pedidoId"); if (pedidoIdStr==null) { response.sendRedirect("/Proyecto_Arreglosapp/Public/admin/administrador-dashboard.jsp"); return;
                        ...
                        out.println("<div style='background:#ffebee;color:#c62828;padding:20px;margin:20px;border-radius:8px;font-family:sans-serif;'><h3>Error Interno (SQL)</h3><p>" + e.getMessage() + "</p></div>"); return; }
                        if (detalle == null) {
                                ...
                                <div class="info-seccion"> <h3 class="info-seccion__titulo">Detalles de la Personalización</h3> 
                                    ...
                                    <% if (personalizacionMaterial != null && !personalizacionMaterial.isEmpty()) { %>
                                        <div class="info-seccion__campo info-seccion__campo--inline"><span class="info-seccion__label">Material:</span><span class="info-seccion__valor">
                                                    <%= personalizacionMaterial %>
                                                </span></div>
                                    <% } %>
                                    <div class="info-seccion__campo"><span class="info-seccion__label">Descripción:</span><span class="info-seccion__valor" style="display:block;margin-top:5px;line-height:1.4;">
                                                <%= personalizacionDescripcion %>
                                            </span></div>
                                </div>
                                <% } %>
