...
<%
    ...
    /* ── Carga de datos desde la capa DAO ── */
...
<body class="grid-principal">
    ...
    <main class="dashboard" id="contenido-principal" role="main">
        ...
        <section class="dashboard__estadisticas">
            <article class="estadistica" onclick="mostrarVista('pedidos')" style="cursor:pointer;">
                <div class="estadistica__contenido">
                    <span class="estadistica__valor"><%= totalPedidosActivos %></span>
                    <img class="estadistica__icono" src="../../Assets/icons/hacia-adelante.png" alt="">
                </div>
                <span class="estadistica__label">Pedidos activos</span>
            </article>

            <article class="estadistica" onclick="mostrarVista('citas')" style="cursor:pointer;">
                <div class="estadistica__contenido">
                    <span class="estadistica__valor"><%= totalCitasHoy %></span>
                    <img class="estadistica__icono" src="../../Assets/icons/hacia-adelante.png" alt="">
                </div>
                <span class="estadistica__label">Citas hoy</span>
            </article>

            <article class="estadistica" onclick="mostrarVista('todasCitas')" style="cursor:pointer;">
                <div class="estadistica__contenido">
                    <span class="estadistica__valor"><%= totalTodasCitas %></span>
                    <img class="estadistica__icono" src="../../Assets/icons/hacia-adelante.png" alt="">
                </div>
        ...
        <section class="dashboard__seccion-pedidos" id="vistaPedidos">
            ...
            <div class="pedidos__encabezado-filtro"
                ...
                <form action="administrador-dashboard.jsp" method="get" class="pedidos__filtros"
                    ...
                    <select name="fEstado" class="pedidos__filtro-btn"
                        ...
                        <option value="terminado"  <%= "terminado".equals(fEstadoActual)  ? "selected" : "" %>>Terminado</option>
                    </select>
                    <button type="submit" class="pedidos__filtro-btn pedidos__filtro-btn--activo"
                    ...
                    <% if (fEstadoActual != null && !fEstadoActual.isEmpty()) { %>
                    <a href="administrador-dashboard.jsp" class="pedidos__filtro-btn"
            ...
            <!-- Lista de pedidos -->
            <div class="pedidos__lista" id="listaPedidos" style="margin-top:12px;">
                <% if (pedidosRecientes == null || pedidosRecientes.isEmpty()) { %>
                <p style="color:#888;font-size:13px;text-align:center;padding:20px;">No hay pedidos</p>
                <% } %>

                <% for (int i = 0; i < pedidosRecientes.size(); i++) {
                    Map<String, Object> pedido = pedidosRecientes.get(i);
                    int    pedidoId   = (Integer) pedido.get("pedidoId");
                    String estado     = (String)  pedido.get("estado");
                    String citaEstado = (String)  pedido.get("citaEstado");
                    ...
                    /* Clase y etiqueta de estado del pedido */
                    String estadoClass = "pedido__estado--pendiente";
                    String estadoLabel = "Pendiente de Revision";
                    if ("confirmado".equals(estado))  { estadoClass = "pedido__estado--confirmado"; estadoLabel = "Pendiente de Inicio"; }
                    else if ("terminado".equals(estado))  { estadoClass = "pedido__estado--entregado";  estadoLabel = "Terminado"; }
                    else if ("cancelado".equals(estado))  { estadoClass = "pedido__estado--cancelado";  estadoLabel = "Cancelado"; }

                    /* data-estado para el filtro visual por JS (evita comillas dentro de atributo HTML) */
                    String citaEstadoData = (citaEstado != null) ? citaEstado : "programada";
                    pageContext.setAttribute("pedidoId", pedidoId);
                %>
                <article class="pedido" data-estado="<%= citaEstadoData %>">
                    <div class="pedido__contenido">
                        <span class="pedido__id">ID: #P-<%= String.format("%05d", pedidoId) %></span>
                    ...
                    <div class="pedido__acciones">
                        <span class="pedido__estado <%= estadoClass %>"><%= estadoLabel %></span>
                        <a href="${pageContext.request.contextPath}/Public/admin/detalle-pedido-admin.jsp?pedidoId=${pedidoId}" class="pedido__enlace">Detalles</a>
                    </div>
                </article>
                <% } %>
            </div>
        ...
        <!-- ── VISTA: CITAS DE HOY ── -->
        <section class="dashboard__seccion-pedidos" id="vistaCitas" style="display:none;">
            <h2 class="pedidos__titulo">Citas de Hoy</h2>
            <div class="pedidos__lista">
                <% if (citasHoy == null || citasHoy.isEmpty()) { %>
                <p style="color:#888;font-size:13px;text-align:center;padding:20px;">No hay citas para hoy</p>
                <% } %>

                <% for (int j = 0; j < citasHoy.size(); j++) {
                    Map<String, Object> cita = citasHoy.get(j);
                    int    citaId      = (Integer) cita.get("citaId");
                    String clienteCita = (String)  cita.get("cliente");
                    String estadoCita  = (String)  cita.get("estado");
                    String notasCita   = (String)  cita.get("notas");
                    String motivoCita  = (String)  cita.get("motivo");
                    java.sql.Timestamp fechaHoraCita = (java.sql.Timestamp) cita.get("fechaHora");
                    String horaStr = (fechaHoraCita != null) ? sdfHora.format(fechaHoraCita) : "Sin hora";
                    ...
                    /* Clase y etiqueta de estado de la cita */
                    String estadoCitaClass = "pedido__estado--pendiente";
                    String estadoCitaLabel = "Programada";
                    if ("confirmada".equals(estadoCita))  { estadoCitaClass = "pedido__estado--confirmado"; estadoCitaLabel = "Confirmada"; }
                    else if ("completada".equals(estadoCita)) { estadoCitaClass = "pedido__estado--entregado";  estadoCitaLabel = "Completada"; }
                    else if ("cancelada".equals(estadoCita))  { estadoCitaClass = "pedido__estado--cancelado";  estadoCitaLabel = "Cancelada"; }

                    boolean citaFinalizada = "completada".equals(estadoCita) || "cancelada".equals(estadoCita);
                %>
                <article class="pedido">
                    <div class="pedido__contenido">
                        ...
                        <div class="pedido__info-cliente">
                            <div class="pedido__cliente-row">
                                <span class="pedido__label">Cliente:</span>
                                <span class="pedido__cliente"><%= clienteCita %></span>
                            </div>
                            <% if (notasCita != null && !notasCita.trim().isEmpty()) { %>
                            <span class="pedido__cita"><%= notasCita %></span>
                            <% } %>
                        </div>
                    </div>
                    <div class="pedido__acciones">
        ...
        <section class="dashboard__seccion-pedidos" id="vistaTodasCitas" style="display:none;">
            <div class="pedidos__encabezado-filtro"
                style="flex-direction:column;align-items:flex-start;gap:15px;margin-bottom:25px;">
                <h2 class="pedidos__titulo">Historial y Agenda Completa</h2>

                <!-- Formulario de filtro por fecha y cliente -->
                <form action="administrador-dashboard.jsp" method="get" class="pedidos__filtros"
                    style="display:flex;flex-wrap:wrap;gap:12px;width:100%;">
                    ...
                    <input type="date" name="fFecha"
                        ...
                        style="padding:8px 15px;border-radius:25px;border:1px solid #e0e0e0;background:white;font-size:14px;outline:none;">
                    <input type="text" name="fCliente" placeholder="Nombre cliente..."
                        value="<%= request.getParameter("fCliente") != null ? request.getParameter("fCliente") : "" %>"
                        class="pedidos__filtro-btn"
                        style="padding:8px 15px;border-radius:25px;border:1px solid #e0e0e0;background:white;font-size:14px;flex-grow:1;outline:none;">
                    <button type="submit" class="pedidos__filtro-btn pedidos__filtro-btn--activo"
                    ...
                    <a href="administrador-dashboard.jsp?vista=todasCitas" class="pedidos__filtro-btn"
                        style="padding:8px 18px;border-radius:25px;border:1px solid #673ab7;color:#673ab7;background:transparent;text-decoration:none;font-weight:500;display:inline-flex;align-items:center;">Limpiar</a>
                </form>
            </div>
            ...
            <div class="pedidos__lista">
                ...
                <p style="color:#888;font-size:13px;text-align:center;padding:20px;">No hay citas registradas</p>
                <% } %>

                <% for (int k = 0; k < todasLasCitas.size(); k++) {
                    Map<String, Object> cita2 = todasLasCitas.get(k);
                    int    citaId2      = (Integer) cita2.get("citaId");
                    String clienteCita2 = (String)  cita2.get("cliente");
                    String estadoCita2  = (String)  cita2.get("estado");
                    String notasCita2   = (String)  cita2.get("notas");
                    java.sql.Timestamp fechaHoraCita2 = (java.sql.Timestamp) cita2.get("fechaHora");
                    String fechaStr2 = (fechaHoraCita2 != null) ? sdf.format(fechaHoraCita2) : "Sin fecha";

                    /* Clase y etiqueta de estado */
                    String estadoCitaClass2 = "pedido__estado--pendiente";
                    String estadoCitaLabel2 = "Programada";
                    if ("confirmada".equals(estadoCita2))  { estadoCitaClass2 = "pedido__estado--confirmado"; estadoCitaLabel2 = "Confirmada"; }
                    else if ("completada".equals(estadoCita2)) { estadoCitaClass2 = "pedido__estado--entregado";  estadoCitaLabel2 = "Completada"; }
                    else if ("cancelada".equals(estadoCita2))  { estadoCitaClass2 = "pedido__estado--cancelado";  estadoCitaLabel2 = "Cancelada"; }

                    boolean citaFinalizada2 = "completada".equals(estadoCita2) || "cancelada".equals(estadoCita2);
                %>
                <article class="pedido">
                    <div class="pedido__contenido">
                        <span class="pedido__id"><%= fechaStr2 %></span>
                        <div class="pedido__info-cliente">
                            <div class="pedido__cliente-row">
                                <span class="pedido__label">Cliente:</span>
                                <span class="pedido__cliente"><%= clienteCita2 %></span>
                            </div>
                            <% if (notasCita2 != null && !notasCita2.trim().isEmpty()) { %>
                            <span class="pedido__cita"><%= notasCita2 %></span>
                            <% } %>
                        </div>
                    </div>
                    <div class="pedido__acciones">
    ...
    <!-- ── MODAL: GESTIONAR ESTADO DE CITA ── -->
    <div id="modalCita" class="modal-overlay" style="display:none;">
        <div class="modal-contenido">
            <h2 class="modal__titulo">Gestionar Cita</h2>
            <p class="modal__descripcion" id="modalCitaDescripcion">Selecciona una accion</p>
            <div class="modal-cita__opciones" id="modalCitaBotones"></div>
            <button class="btn-modal btn-modal--cancelar" style="width:100%;margin-top:12px;"
                onclick="cerrarModalCita()">Cerrar</button>
        </div>
    </div>

    <!-- Formulario oculto que el modal usa para enviar el cambio de estado -->
    <form method="post" action="/Proyecto_Arreglosapp/AdminServlet" id="formCita" style="display:none;">
    ...
    <script>
        /* ── Controla qué sección del dashboard se muestra ── */
        ...
        /* ── Abre el modal con las acciones disponibles según el estado de la cita ── */
